...
const CommentArea = ({ post }) => {
  ...
  const toggleCommentInput = () => {
    if (!selectedPostId) {
      // If the user wants to add comment and open the commentbox
  ...
  const removeSelectedPostId = () => {
    if (selectedPostId === post?._id) {
      // if user wants to close the commentbox
      setSelectedPostId('');
      dispatch(clearPost());
    } else {
      // if user wants to open another commentbox to add comment on another post
      setSelectedPostId(post?._id);
      dispatch(updatePostItem(post));
    }
  };
    ...
    try {
      ...
      if (!Object.keys(reactionResponse.data.reactions).length) {
        ...
        if (reaction === reactionsData.previousReaction) {
          // remove reaction if user already have same reaction on the post
  ...
  // to emit socketIO 'reaction', this will not go to db it just goes to socket and frontend is updated
